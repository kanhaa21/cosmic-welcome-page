...
export default function BlackHolePage() {
  ...
  return (
    <main ref={containerRef} className="relative min-h-[300vh] bg-[#020202] text-white selection:bg-orange-500/30">
      <CustomCursor />
      <Taskbar />
      ...
      <section ref={heroRef} className="relative z-10 h-screen flex flex-col items-center justify-center px-4">
        <div className="text-center">
          <motion.span 
            ...
            className="text-orange-500 font-black uppercase text-[10px] md:text-xs mb-8 block"
          >
          ...
          <h1 className="hero-title flex flex-col text-7xl md:text-[12rem] font-black tracking-tighter leading-[0.85] mb-12">
            ...
            <span className="block overflow-hidden bg-gradient-to-r from-orange-400 via-white to-orange-400 bg-clip-text text-transparent">MASSIVE</span>
          </h1>
          <motion.p 
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
        ...
        <div className="max-w-7xl mx-auto">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {DATA_POINTS.map((point, i) => (
              <div 
                key={i} 
                className="data-card group relative p-8 rounded-3xl bg-white/[0.03] border border-white/10 backdrop-blur-xl hover:bg-white/[0.05] transition-all duration-500"
              >
                <div className={`absolute inset-0 bg-gradient-to-br ${point.color} to-transparent opacity-0 group-hover:opacity-100 transition-opacity rounded-3xl`} />
                <span className="text-orange-500/50 font-mono text-sm mb-4 block">0{i + 1}</span>
                <h3 className="text-2xl font-bold mb-4">{point.title}</h3>
                <p className="text-zinc-400 font-light leading-relaxed mb-8">
                  {point.description}
                </p>
                <div className="stat-value pt-6 border-t border-white/10">
                  <span className="text-[10px] uppercase tracking-widest text-zinc-500 block mb-1">Observation Data</span>
                  <span className="text-sm font-mono text-orange-400">{point.stat}</span>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="mt-32 grid grid-cols-1 md:grid-cols-2 gap-24 items-center">
            <div className="space-y-8">
              <h2 className="text-4xl md:text-6xl font-bold leading-tight">
                Beyond the <br />
                <span className="italic font-serif text-orange-500">Known Physics</span>
              </h2>
              ...
              <div className="flex gap-4">
                <Link 
                  href="/"
                  className="px-8 py-4 rounded-full bg-white text-black font-bold text-xs uppercase tracking-widest hover:bg-orange-500 hover:text-white transition-all"
                >
                  Return to Origin
                </Link>
              </div>
            ...
            <div className="relative aspect-square rounded-full border border-orange-500/20 flex items-center justify-center">
              <div className="absolute inset-0 animate-spin-slow rounded-full border border-dashed border-orange-500/10" />
              <div className="w-3/4 h-3/4 rounded-full bg-orange-500/5 blur-3xl animate-pulse" />
